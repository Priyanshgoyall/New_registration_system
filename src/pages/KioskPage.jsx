...
import { normalizePhone, normalizeName, normalizeFatherName, normalizeClass, normalizeSchool, normalizeCity } from '../utils/speechNormalize';
...
const VOICE_FIELDS = [
  { key: 'name', label: 'Full Name', icon: User, placeholder: 'e.g. Priyansh Goyal', hint: 'Say your full name (e.g. "My name is Priyansh Goyal")', required: true },
  { key: 'father_name', label: "Father's Name (Optional)", icon: UserCheck, placeholder: 'e.g. Rajesh Goyal (Optional)', hint: 'Say father\'s name or say "skip" if not available', required: false },
  { key: 'phone', label: 'Phone Number (Optional)', icon: Phone, placeholder: 'e.g. 9876543210 (Optional)', hint: 'Say 10 digit phone number or say "skip"', required: false },
  { key: 'class', label: 'Class / Standard', icon: School, placeholder: 'e.g. 10th, 12th, B.Tech 1st Year', hint: 'Say your class or grade', required: true },
  { key: 'school_name', label: 'School / College Name', icon: School, placeholder: 'e.g. Delhi Public School', hint: 'Say your school or college name', required: true },
  { key: 'city', label: 'City', icon: MapPin, placeholder: 'e.g. Bhopal, New Delhi', hint: 'Say your city name', required: true },
];
...
export default function KioskPage() {
  ...
  // ── Form state ──
  const [form, setForm] = useState({ sessionId: '', name: '', father_name: '', phone: '', class: '', school_name: '', city: '' });
  ...
  const normalizeForField = useCallback((fieldKey, transcript) => {
    ...
    if (fieldKey === 'father_name') return normalizeFatherName(transcript);
    if (fieldKey === 'phone') return normalizePhone(transcript);
    if (fieldKey === 'class') return normalizeClass(transcript);
  ...
  const listenForField = useCallback((fieldIndex) => {
    ...
    recognition.onresult = (event) => {
      ...
      const isSkip = lowerTranscript.includes('skip') || lowerTranscript.includes('no') || lowerTranscript.includes('none') || lowerTranscript.includes("don't have");

      // Strict validation for captured voice field
      let isValid = true;
      let errorMsg = '';
      let valueToSet = cleaned;
      ...
      if (field.key === 'phone') {
        if (isSkip) {
          ...
          if (!digitsOnly || digitsOnly.length !== 10) {
            isValid = false;
            errorMsg = 'Please say a 10-digit phone number, or say "skip" if not available.';
          }
        }
      } else if (field.key === 'father_name') {
        if (isSkip) {
          valueToSet = '';
        }
        ...
        if (!cleaned || cleaned.trim().length === 0) {
          isValid = false;
          errorMsg = 'Please say your class clearly (e.g. 10th or 12th).';
        }
      ...
      setForm((f) => ({ ...f, [field.key]: valueToSet }));
  ...
  const validateDetails = () => {
    ...
    if (form.phone.trim()) {
      ...
      if (phoneDigits.length !== 10) errors.phone = 'If provided, phone must be exactly 10 digits.';
    }
    if (!form.class.trim()) errors.class = 'Class / Standard is required.';
    if (!form.school_name.trim()) errors.school_name = 'School / College Name is required.';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Reject obviously invalid phone patterns if phone is provided
      if (phoneDigits) {
      ...
      // RPC Duplicate check for name, father_name, phone
      const safeName = sanitizeText(form.name);
      const safeFatherName = sanitizeText(form.father_name);
      const safePhone = phoneDigits;
      const safeClass = sanitizeText(form.class);
      ...
      if (!dupErr && dupCheck) {
        ...
        if (dupCheck.name_exists) {
          toast.error(`A student with name "${safeName}" is already registered for this session (ID: ${dupCheck.existing_reg_id}).`, { duration: 7000 });
          setSubmitting(false);
          return;
        }
      }
    ...
    <div className="h-screen bg-slate-50 flex flex-col overflow-hidden">
      ...
      <header className="bg-white border-b border-slate-200 shadow-sm flex-shrink-0">
        <div className="max-w-6xl mx-auto flex items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-slate-900 leading-tight">SPORTS REGISTRATION PORTAL</h1>
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden min-h-0">
        ...
        <aside className="hidden lg:flex flex-col justify-center bg-gradient-to-b from-blue-700 to-blue-800 text-white px-10 py-12 lg:w-80 xl:w-96 flex-shrink-0 h-full overflow-hidden">
          ...
          <h2 className="text-2xl font-bold mb-3">Sports Registration</h2>
          ...
          <div className="space-y-4">
            {[
              ...
              { step: '3', label: 'Fill / Voice Details', desc: 'Name, Father Name, Phone, Class, School, City' },
          ...
          <div className="mt-8 pt-5 border-t border-blue-600/40 text-xs text-blue-200/90 space-y-1">
            <p className="font-bold text-white text-xs">Sports Registration Portal</p>
