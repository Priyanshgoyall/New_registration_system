...
export default function Students() {
  ...
  const [editForm, setEditForm] = useState({ name: '', father_name: '', phone: '', class: '', school_name: '', city: '' });
  ...
  const openView = async (student) => {
    ...
    const { data } = await supabase
      .from('registrations')
      .select('registration_id, father_name, class, city, registration_status, registered_at, sessions(name)')
  ...
  const handleSaveEdit = async () => {
    if (!editForm.name.trim() || !editForm.school_name.trim() || !editForm.class.trim()) {
  ...
  const handleExportCSV = () => {
    ...
    exportToCSV(rows, ['name', 'father_name', 'class', 'phone', 'school_name', 'city', 'joined'], 'students-export');
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search by name, father's name, class, phone, city, or school…"
          value={search}
            ...
            <table className="w-full">
              <thead>
                <tr>
                  ...
                  <th className="table-header hidden sm:table-cell">Father's Name</th>
                  <th className="table-header hidden md:table-cell">Class</th>
                  <th className="table-header hidden lg:table-cell">City / Phone</th>
              ...
              <tbody>
                {paginated.map((s) => (
                  <tr key={s.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="table-cell hidden sm:table-cell text-xs text-slate-600">{s.father_name || '—'}</td>
                    <td className="table-cell hidden md:table-cell text-xs text-slate-600">{s.class || '—'}</td>
          ...
          <div className="space-y-5">
            ...
            {editing ? (
              <div className="space-y-3">
                ...
                <div>
                  <label className="label"><User size={12} className="inline mr-1" />Father's Name (Optional)</label>
                  <input className="input-field" value={editForm.father_name} onChange={(e) => setEditForm((f) => ({ ...f, father_name: e.target.value }))} />
                </div>
                <div>
                  <label className="label"><School size={12} className="inline mr-1" />Class / Standard</label>
                  <input className="input-field" value={editForm.class} onChange={(e) => setEditForm((f) => ({ ...f, class: e.target.value }))} />
                </div>
                <div>
                  <label className="label"><Phone size={12} className="inline mr-1" />Phone (Optional)</label>
                  <input className="input-field" value={editForm.phone} onChange={(e) => setEditForm((f) => ({ ...f, phone: e.target.value }))} type="tel" />
                </div>
                <div>
                  <label className="label"><School size={12} className="inline mr-1" />School / College Name</label>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-slate-50 rounded-xl p-4 space-y-3 text-sm border border-slate-200">
                  <div className="flex items-center gap-3 text-slate-600">
                    ...
                    <span>Father: {viewing.father_name || 'Not provided'}</span>
                  </div>
                  <div className="flex items-center gap-3 text-slate-600">
                    <School size={14} className="text-slate-400 flex-shrink-0" />
                    <span>Class: {viewing.class || 'Not provided'}</span>
                  </div>
                  <div className="flex items-center gap-3 text-slate-600">
                    <Phone size={14} className="text-slate-400 flex-shrink-0" />
                    <span>Phone: {viewing.phone || 'Not provided'}</span>
                  </div>
                  <div className="flex items-center gap-3 text-slate-600">
                    <School size={14} className="text-slate-400 flex-shrink-0" />
                    <span>School: {viewing.school_name}</span>
                  </div>
                  <div className="flex items-center gap-3 text-slate-600">
                    <MapPin size={14} className="text-slate-400 flex-shrink-0" />
                    <span>City: {viewing.city || 'Not provided'}</span>
                  </div>
                </div>
