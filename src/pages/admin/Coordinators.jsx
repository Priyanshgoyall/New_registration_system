...
CREATE POLICY "auth_all_user_profiles" ON public.user_profiles
  ...
  const fetchData = async () => {
    ...
    const [{ data: profs, error: profErr }, { data: regs, error: regErr }] = await Promise.all([
      ...
      supabase
        .from('registrations')
        .select('id, registration_id, registered_at, registration_status, class, father_name, coordinator_id, student_id, students(name, father_name, phone, class, school_name, city), sessions(name)')
