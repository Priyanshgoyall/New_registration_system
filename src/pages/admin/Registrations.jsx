...
export default function Registrations() {
  ...
  const getRows = () => filtered.map((r) => ({
    ...
    father_name: r.students?.father_name || r.father_name || '',
    class: r.students?.class || r.class || '',
    phone: r.students?.phone || r.phone || '',
  ...
  const cols = ['name', 'father_name', 'class', 'phone', 'school_name', 'city', 'session', 'registration_id', 'status', 'registered_at'];
            ...
            <table className="w-full">
              <thead>
                <tr>
                  ...
                  <th className="table-header hidden sm:table-cell">Father / Class</th>
                  <th className="table-header hidden md:table-cell">City / Phone</th>
              ...
              <tbody>
                {paginated.map((r) => (
                  <tr key={r.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="table-cell hidden sm:table-cell text-slate-600 text-xs">
                      <p className="font-medium text-slate-800">{r.students?.father_name || r.father_name || '—'}</p>
                      <p className="text-[11px] text-slate-400">Class: {r.students?.class || r.class || '—'}</p>
                    </td>
                    <td className="table-cell hidden md:table-cell text-slate-600 text-xs">
                      <p>{r.students?.city || r.city || '—'}</p>
                      <p className="text-[11px] text-slate-400">{r.students?.phone || r.phone || '—'}</p>
                    </td>
